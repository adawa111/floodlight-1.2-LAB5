...
public class StatisticsCollector implements IFloodlightModule, IStatisticsService {
	...
	protected double PortTxThreshold;
	protected double PortRxThreshold;
	...
	private class FlowStatCollector implements Runnable {


		public void start() {
			portStatsCollector = executor.scheduleAtFixedRate(this, 0, interval, TimeUnit.SECONDS);
		}

		public void stop() {
			if (portStatsCollector != null) {
				portStatsCollector.cancel(true);
			}
		}
		...
		public void run() {
			for (IOFSwitch sw : switchService.getAllSwitchMap().values()) {
				OFFlowStatsReply flowStats = sendFlowStatsRequest(sw);
				if (flowStats != null) {
					processFlowStats(sw.getId(), flowStats);
				}
			}
		}
		private OFFlowStatsReply sendFlowStatsRequest(IOFSwitch sw) {
			...
			try {
				...
				List<OFFlowStatsReply> replies = sw.writeStatsRequest(flowStatsRequest).get();

				for (OFStatsReply reply : replies) {
					if (reply.getStatsType() == OFStatsType.FLOW) {
						return (OFFlowStatsReply) reply;
					}
				}
			} catch (Exception e) {
				log.error("Error al enviar la solicitud de estadísticas de flujo: {}", e.getMessage());
			}
			return null;
		}
		private void processFlowStats(DatapathId dpid, OFFlowStatsReply flowStats) {
			for (OFFlowStatsEntry entry : flowStats.getEntries()) {

				long byteCount = entry.getByteCount().getValue();
				long packetCount = entry.getPacketCount().getValue();

				log.info("DPID: {}", dpid);
				log.info("Flow Entry: Byte Count = {}",byteCount);
				log.info("Flow Entry: Packet Count = {}", packetCount);
			}
		}
	}
	 ...
	 */
		private void loadThresholdsFromPropertiesFile() {
			...
			try {
				InputStream input = new FileInputStream("floodlightdefault.properties");
				properties.load(input);
				PortTxThreshold = Double.parseDouble(properties.getProperty("PortTxThreshold"));
				PortRxThreshold = Double.parseDouble(properties.getProperty("PortRxThreshold"));
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
	 ...
	 */
		...
		public SwitchPortBandwidth getBandwidthConsumption(DatapathId dpid, OFPort p) {
			SwitchPortBandwidth portBandwidth = portStats.get(new NodePortTuple(dpid, p));

			// Comparar con los umbrales
			if (portBandwidth != null) {
				double txBandwidth = portBandwidth.getBitsPerSecondTx().getValue();
				double rxBandwidth = portBandwidth.getBitsPerSecondRx().getValue();

				if (txBandwidth > PortTxThreshold) {
					System.out.println("El ancho de banda de TX ha superado el umbral en el puerto " + p);
				}

				if (rxBandwidth > PortRxThreshold) {
					System.out.println("El ancho de banda de RX ha superado el umbral en el puerto " +p);
				}
			}
	 ...
	 */
		...
		private void startStatisticsCollection() {
			...
			flowStatsCollector = threadPoolService.getScheduledExecutor().scheduleAtFixedRate(new FlowStatCollector(),portStatsInterval,portStatsInterval,TimeUnit.SECONDS);
		...
		private void stopStatisticsCollection() {
			...
			if (!flowStatsCollector.cancel(false)) {
				log.error("Could not cancel flows stats thread");
			} else {
				log.warn("Statistics collection thread(s) stopped");
			}
		}
